...
export const Header: React.FC<HeaderProps> = ({ onOpenModal, cartItemCount, onCartClick }) => {
    ...
    const NavLinks: React.FC<{mobile?: boolean}> = ({ mobile = false }) => {
        ...
        const desktopClass = `text-base font-medium rounded-lg px-4 py-2 border border-surface transition-all duration-300 hover:bg-accent hover:text-text-on-accent hover:border-accent`;

        return (
            <>
                <a href="#services" onClick={handleNavClick} className={mobile ? mobileClass : desktopClass}>Услуги</a>
                <a href="#cosmetics" onClick={handleNavClick} className={mobile ? mobileClass : desktopClass}>Уход Дома</a>
                <a href="#price-calc" onClick={handleNavClick} className={mobile ? mobileClass : desktopClass}>Комплекс</a>
                <a href="#gift-certificates" onClick={handleNavClick} className={mobile ? mobileClass : desktopClass}>Сертификаты</a>
                <a href="#contacts" onClick={handleNavClick} className={mobile ? mobileClass : desktopClass}>Контакты</a>
            </>
        );
    };
